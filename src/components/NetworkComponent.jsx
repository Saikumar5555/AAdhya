...
const NetworkComponent = () => {
  ...
  useEffect(() => {
    const sketch = (p) => {
      ...
      p.draw = () => {
        p.background(0); // Change background to black
        network.update(p);
        network.display(p);
      ...
      function Neuron(x, y, p, color) {
        ...
        const display = () => {
          ...
          p.fill(color === 'violet' ? 'violet' : 'green'); // Change color based on neuron
      ...
      function Connection(from, to, weight) {
        ...
        const display = () => {
          p.stroke(255); // Change the color of connections to white
